...
class RangeService(object):
    ...
    def load_all_fund_perform_info(self):
        ...
        if(len(self.fund_codes_list) > 0 and len(self.fund_performance_list) <= 0):
            print('loading funds...')
            self.fund_performance_list = [self.mp_service.get_performance_on_fund(f) for f in self.fund_codes_list]
            print('funds loaded')
        return self.fund_performance_list

    def find_best_top500_funds(self, fund_category = 0):
        '''
        To return the funds and the managers
        '''
        self.load_all_fund_perform_info()
        good_fund_list = list(filter(lambda f:(f.suggestion == '建议增持' or f.suggestion == '建议买入'), self.fund_performance_list))
        best_funds_top500 = sorted(good_fund_list, key=lambda f: f.score, reverse=True)[0:499]
        return best_funds_top500

    def find_worst_top500_funds(self):
        '''
        performance.suggestion == '建议减持' / '建议观望'
        '''
        self.load_all_fund_perform_info()
        bad_fund_list = list(filter(lambda f:(f.suggestion == '建议减持' or f.suggestion == '建议观望'), self.fund_performance_list))
